...
def test_import_schema(monkeypatch, schema_json, schema_file, expected):
    monkeypatch.setattr(
        "builtins.open", lambda x, y: StringIO(TEST_SCHEMA_FILE_CONTENT))
...
def test_avro_schema_decorator(monkeypatch):
    ...
    monkeypatch.setattr(
        "builtins.open", lambda x, y: StringIO(TEST_SCHEMA_FILE_CONTENT))
    monkeypatch.setattr(
        "avro_models.core.SchemaFromJSONData", lambda x, y: schema)
    monkeypatch.setattr(
        container, "register_model", lambda x, y: TEST_OUTPUT_CLASS)
...
def test_register_model(monkeypatch):
    ...
    monkeypatch.setattr(
        "avro_models.core.create_data_model",
